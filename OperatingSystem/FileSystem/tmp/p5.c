...
int my_mkdir(char *path) {
  ...
  if (parent_disk_num == -1) return -1;      /* previous dir not existed */
  ...
  if (tmpDir.in_use < MAX_DIRENTRY) {
    ...
    if (block_location == -1) return -1; /* not success */
    ...
    printf("make dir success: %s i-node in: %d", PathName[path_num - 1],
           block_location);
    return 0;
  }
  return -1;
...
int my_rmdir(char *path) {
  int path_num, i, disk_num, dir_in_use;
  char TmpBuf[BLOCKSIZE];
  DIR tmpDir;
  path_num = parse_path(path);
  disk_num = find_file(path_num);            /* find last  dir */
  Parent_disk_num = find_file(path_num - 1); /* find parent  dir */
  if (disk_num == -1) return -1;             /* previous dir not existed */
  /* read dir from disk */
...
void my_mkfs() {
  ...
  if (read_block(1, TmpBuf) == 0) /* read first block */
  {
    ...
    if (SuperBlock.is_format != 1) {
      ...
      for (int i = 1; i <= BITMAP_NUM; i++) SuperBlock.bitmap_usage[i] = 0;
      ...
      for (int i = 2; i <= 57; i++) {
        if (read_block(i, TmpBuf) == 0) {
          ...
          if (i == 2) {
            ...
            for (int j = 0; j <= 57; j++) bitmapTmp.bitmap[j] = 1;
